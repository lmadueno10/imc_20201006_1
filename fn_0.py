...
def mysql_conn():
    global db
    try:
        db = mysql.connector.connect(host="localhost", user="root", password="sasa", database="imc")
...
def find_by_part_number(path_file):
    ...
    for header in document.iter('Header'):
        ...
        #BlanketEffectiveDate = header.find('BlanketEffectiveDate').text
        ParentAAIAID = header.find('ParentAAIAID').text
        BrandOwnerAAIAID = header.find('BrandOwnerAAIAID').text
        #CurrencyCode = header.find('CurrencyCode').text
        ...
        for item in items:
            ...
            for replacements in item.iter('PartInterchangeInfo'):
                for replacement in replacements:
                    _BrandAAIAID = replacement.attrib['BrandAAIAID'] #replacement.find('BrandAAIAID').text
